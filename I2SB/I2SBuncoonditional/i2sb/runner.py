...
class Runner(object):
    def __init__(self, opt, log, save_opt=True):
        ...
        self.log = log
        self.ssim_fn = torchmetrics.StructuralSimilarityIndexMeasure(data_range=1.0).to(opt.device)# saurabh validation ssim
        self.psnr_fn = torchmetrics.PeakSignalNoiseRatio(data_range=1.0).to(opt.device)# saurabh validation psnr
    ...
    def train(self, opt, train_dataset, val_dataset, corrupt_method):
        ...
        for it in range(opt.num_itr):
            ...
            for _ in range(n_inner_loop):
                ...
                loss = F.l1_loss(pred, label)#SJ changed to l1 loss
                loss.backward()
            ...
            if it % 10 == 0:
                self.evaluate_ssim_psnr(opt, it, val_loader, corrupt_method) #saurabhJ validation added
        ...
        self.writer.close()
    @torch.no_grad()
    def evaluate_ssim_psnr(self, opt, it, val_loader, corrupt_method): #SJ validation ssim psnr
        """Evaluates SSIM and PSNR for validation set."""
        log = self.log

        x0, x1, mask, y, cond = self.sample_batch(opt, val_loader, corrupt_method)
        img_clean = x0.detach().cpu().numpy()  # Convert to numpy
        img_corrupt = x1.detach().cpu().numpy()  # Convert to numpy

        # SSIM and PSNR between clean and corrupted images
        ssim_value = ssim(img_clean, img_corrupt, data_range=1.0, multichannel=True)
        psnr_value = psnr(img_clean, img_corrupt, data_range=1.0)

        log.info(f"Validation SSIM: {ssim_value:.4f} | PSNR: {psnr_value:.4f}")

        # Log SSIM and PSNR to Tensorboard
        self.writer.add_scalar(it, 'val/SSIM', ssim_value)
        self.writer.add_scalar(it, 'val/PSNR', psnr_value)
